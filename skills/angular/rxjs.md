...
this.users$ = this.http.get<User[]>('/api/users').pipe(
  map(users => users.filter(u => u.id > 10))
);
...
```typescript
import { filter, take, takeUntil, takeWhile, first, last, skip } from 'rxjs/operators';
...
userIds$.pipe(
  concatMap(id => this.http.post('/api/log', { userId: id }))
).subscribe();
...
of('Hello').pipe(
  delay(1000)
).subscribe(console.log); // "Hello" después de 1s
