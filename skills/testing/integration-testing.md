...
@Component({
  ...
  template: `
    <div class="user-manager">
      ...
      <app-user-list [users]="users()" (userDeleted)="onUserDeleted($event)" />
    </div>
  `,
})
...
describe("UserManagerComponent Integration", () => {
  ...
  it("should delete user from list", fakeAsync(() => {
    ...
    const firstDeleteBtn = Array.from(deleteButtons).find((btn) =>
      (btn as HTMLElement).textContent?.includes("Delete"),
    ) as HTMLElement;
  ...
  template: `
    <form [formGroup]="form" (ngSubmit)="onSubmit()">
      ...
      <div formGroupName="passwords">
        ...
        />
        @if (
          form.get("passwords")?.hasError("mismatch") &&
          form.get("passwords")?.touched
        ) {
...
describe("RegistrationFormComponent Integration", () => {
  ...
  it("should show password mismatch error", fakeAsync(() => {
    ...
    const errors = fixture.nativeElement.querySelectorAll(".error");
    const mismatchError = Array.from(errors).find((el) =>
      (el as HTMLElement).textContent?.includes("do not match"),
    );
