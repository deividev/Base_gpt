...
@Component({
  ...
  imports: [CommonModule, Button, FeatureCard, SectionHeader, Input, Stat, Navbar, Footer, DataTable, BarChart],
...
export class Landing {
  ...
  protected readonly currentChartType = signal<'bar' | 'line' | 'pie' | 'doughnut' | 'radar' | 'polarArea' | 'bubble' | 'scatter'>('bar');
  ...
  protected readonly chartOptions = computed(() => {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            const value = context.parsed?.y ?? context.parsed?.r ?? context.raw ?? context.formattedValue;
    ...
    onClick: (event: any, elements: any) => {
      if (elements && elements.length > 0) {
        ...
        if (chartType === 'bubble' || chartType === 'scatter') {
          ...
          if (chartType === 'bubble') {
            this.selectedDataPoint.set(`${datasetLabel}: (x: ${coords.x}, y: ${coords.y}, size: ${coords.r})`);
          } else {
    ...
    scales: this.currentChartType() === 'pie' || this.currentChartType() === 'doughnut' ? undefined : {
      x: {
        grid: {
          color: gridColor,
          drawBorder: false
  ...
  protected changeChartType(type: 'bar' | 'line' | 'pie' | 'doughnut' | 'radar' | 'polarArea' | 'bubble' | 'scatter'): void {
